	...
	if ( is_page_template( 'template-frontpage.php' ) ) {
		...
		wp_localize_script(
			'azera-shop-custom-home', 'viewcart', array(
				'view_cart_label' => esc_html__( '<button type="button" class="btn btn-order next-step">Next<i class="fa fa-angle-right" aria-hidden="true"></i></button>', 'azera-shop' ),
            )
		);
...
/**
 * Custom CBG Code  Added Start here
 */




add_action('woocommerce_before_shop_loop_item', 'my_theme_wrapper_start', 10);
add_action('woocommerce_after_shop_loop_item', 'my_theme_wrapper_end', 10);

function my_theme_wrapper_start() {
if( is_page( 'express-checkout' )){
	
	
   $link = add_query_arg( 'add-to-cart', get_the_ID() );
    echo '<a href="' . $link . '" class="woocommerce-LoopProduct-link next-step">';
...
$post_excerpt = substr($short_description, 0, 200);
...
function wc_ninja_category_is_in_the_cart() {
	...
	$categories = array( 'manufacturer', 'manufacturer' );
...
function wc_ninja_remove_checkout_field( $fields ) {
	...
	unset($fields['billing']['billing_address_2_field']);
	unset($fields['billing']['order_review']);
...
function wc_ninja_remove_checkout_field_def( $fields ) {  
    unset($fields['billing']['billing_address_1']);
    unset($fields['billing']['billing_postcode']);
    unset($fields['billing']['billing_state']);
	unset($fields['billing']['billing_country']);
	...
	unset($fields['billing']['billing_company_field']);
	unset($fields['shipping']['order_comments_field']);
...
add_filter( 'woocommerce_checkout_fields' , 'wc_ninja_remove_checkout_field_def' );
add_filter( 'woocommerce_enable_order_notes_field', '__return_false' );
...
add_filter( 'woocommerce_billing_fields', 'custom_woocommerce_billing_fields' );
 ...
 function custom_woocommerce_billing_fields( $fields ) {
   ...
   $fields['billing_first_name']	= array( 
         'label'    => __('Full Name', 'woothemes'), 

      'placeholder'    => __('Full Name', 'woothemes'), 
      'required'       => true, 
      'class'          => array('form-row update_totals_on_change') 
   );
   $fields['billing_phone']	= array( 
            'label'    => __('MOBILE', 'woothemes'), 

      'placeholder'    => __('MOBILE', 'woothemes'), 
      'required'       => true, 
      'class'          => array('form-row update_totals_on_change') 
   );
    $fields['billing_email']	= array( 
	      'label'    => __('EMAIL ID', 'woothemes'), 

      'placeholder'    => __('EMAIL ID', 'woothemes'), 
      'required'       => true, 
      'class'          => array('form-row update_totals_on_change') 
   );
...
function wc_empty_cart_redirect_url() {
	return '/cbgwp/index.php/express-checkout/';
}
